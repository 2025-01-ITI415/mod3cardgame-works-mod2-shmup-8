...
{
    private static Golf S; // A private Singleton for Golf
    ...
    List<CardGolf> ConvertCardsToCardProspectors(List<Card> listCard)
    {
    ...
    CardGolf Draw()
    {
        CardGolf cp = drawPile[0]; // Pull the 0th CardProspector
    ...
    {                                            // d
        ...
        foreach (CardGolf cg in mine)
        {
            ...
            foreach (int coverID in cg.layoutSlot.hiddenBy)
            {
                ...
                if (coverCG == null || coverCG.state == eCardState.mine)
                {
    ...
    static public void CARD_CLICKED(CardGolf cg)
    {
        ...
        switch (cg.state)
        {
            ...
            case eCardState.mine:
                ...
                int atop = cg.layoutID + 7;
                if (S.mineIdToCardDict.ContainsKey(atop))
                {
                    S.mineIdToCardDict.TryGetValue(atop, out CardGolf atopCard);
                    if (atopCard.state == eCardState.mine)
                    {
                        validMatch = false;
                        break;
                    }
